...
class LSARP:
    ...
    def load_shipments(self, last_repeat=True):
        ...
        if last_repeat: 
            self.shipments = self.shipments.sort_values('RPT').groupby(['PLATE_SETUP', 'PLATE_ROW', 'PLATE_COL'], as_index=False).last()
            self.shipments = self.shipments.sort_values(['PLATE_SETUP', 'PLATE_ROW', 'PLATE_COL'])
