...
class LSARP:
    def __init__(
        ...
        self.protein_groups = MaxQuant(
            path=self.path / engine / "maxquant" / "protein_groups", engine=engine
        )
        self.plex_data = PlexData(
            path=self.path / engine / "plates" / "proteomics", engine=engine
        )
        self.metabolomics_worklist = MetabolomicsWorklist(
            path=self.path / engine / "metabolomics" / "plates", engine=engine
        )
        self.metabolomics_mint = MintResults(
            path=self.path / engine / "metabolomics/mint", engine=engine
        )
    ...
    def morphologies(self):
        ...
        return df.sort_values(["PLATE", "PLATE_ROW", "PLATE_COL"]).reset_index(
            drop=True
        )
    ...
    def add_maxquant(self, path, plate, row, raw_file=None):
        ...
        for fn in expected_files:
            ...
            try:
                ...
                df.assign(PLATE=plate, PLATE_ROW=row, RawFile=raw_file).set_index(
                    ["RawFile", "PLATE", "PLATE_ROW"]
                ).reset_index().to_parquet(fn_out)
    ...
    def add_maxquant_according_to_plex_data(self):
        plex = self.plex_data()
        for ndx, (plate, rpt, row, raw_file) in plex[
            ["PLATE", "RPT", "PLATE_ROW", "RawFile"]
        ].iterrows():
            if (row is None) or (row == "") or isinstance(row, float):
                continue
    ...
    def protein_groups(self):
        ...
        return (
            pd.concat([pd.read_parquet(fn) for fn in fns])
            .sort_values(["PLATE", "PLATE_ROW"])
